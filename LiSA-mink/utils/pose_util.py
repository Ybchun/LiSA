...
def lw_estimate_poses(gt_pc, pred_pc, threshold=0.6, device='cuda'):
    # print(source_pc.shape)
...
def lw_test_estimate_poses(gt_pc, pred_pc, gt_r, gt_t, threshold=0.6, device='cuda'):
    # print(source_pc.shape)
    source_pc = gt_pc.cpu().numpy().reshape(-1, 3)
    target_pc = pred_pc[:, :3].cpu().numpy().reshape(-1, 3)
    gt_r      = gt_r.cpu().numpy().reshape(3, 3)
    gt_t      = gt_t.cpu().numpy().reshape(-1, 3)
    dis_mask = np.sqrt(np.sum(((gt_r@source_pc.T).T + gt_t - target_pc)**2, 1)) <= 2
    pred_t = np.zeros((1, 3))
    pred_q = np.zeros((1, 4))
    num_points = np.sum(dis_mask)
    index1 = np.arange(0, num_points)
    index2 = np.arange(0, num_points)
    # np.random.shuffle(index1)
    index1 = np.expand_dims(index1, axis=1)
    ...
    source = open3d.geometry.PointCloud()
    target = open3d.geometry.PointCloud()
    source.points = open3d.utility.Vector3dVector(source_xyz[dis_mask, :])
    target.points = open3d.utility.Vector3dVector(target_xyz[dis_mask, :])
    corres = open3d.utility.Vector2iVector(corr)
    ...
    pred_q[0, :] = txq.mat2quat(M.transformation[:3, :3])
    R            = M.transformation[:3, :3]
    # T            = post_refinement(pred_trans, gt_pc.view(1, -1, 3), pred_pc.view(1, -1, 3))
    # R            = T[:3, :3]
    # pred_t       = T[:3, 3]
    # pred_q       = txq.mat2quat(R)
    dismat       = (R@source_xyz.T).T + pred_t - target_xyz
    dismat       = np.sqrt(np.sum(dismat**2, 1))

    return pred_t, pred_q, dismat
