...
class CollationFunctionFactory:
    def __init__(self, collation_type='default'):
        ...
        elif collation_type == 'pgo_collate_pair':
            self.collation_fn = self.pgo_collate_pair_fn
        else:
    ...
    def pgo_collate_pair_fn(self, list_data):
        N = len(list_data)
        list_data = [data for data in list_data if data is not None]
        if N != len(list_data):
            logging.info(f"Retain {len(list_data)} from {N} data.")
        ...
        coords, feats, coords_s8, feats_s8, rot, pose, vo_pose = list(zip(*list_data))
        ...
        rot_batch = torch.from_numpy(np.stack(rot)).float()
        pose_batch = torch.from_numpy(np.stack(pose)).float()
        vo_pose_batch = torch.from_numpy(np.stack(vo_pose)).float()
