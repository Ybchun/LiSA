...
class RobotCar(data.Dataset):
    def __init__(self, data_path, train=True, valid=False, augmentation=[],  voxel_size=0.3, real=False,
        ...
        self.min_volume_space = [-60, -60, -2]
        self.max_volume_space = [60, 60, 6]
    ...
    def __getitem__(self, index):
        ...
        ptcld = load_velodyne_binary(scan_path).reshape(4, -1).transpose()
        ptcld[:, 2] = -1 * ptcld[:, 2]
        # print(ptcld.shape)
        ...
        mask_x = np.logical_and(scan[:, 0] > self.min_volume_space[0], scan[:, 0] < self.max_volume_space[0])
        mask_y = np.logical_and(scan[:, 1] > self.min_volume_space[1], scan[:, 1] < self.max_volume_space[1])
        mask_z = np.logical_and(scan[:, 2] > self.min_volume_space[2], scan[:, 2] < self.max_volume_space[2])
